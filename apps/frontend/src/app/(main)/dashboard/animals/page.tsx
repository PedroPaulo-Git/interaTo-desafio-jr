...
export default function MyAnimalsPage() {
    ...
    useEffect(() => {
        const fetchAnimals = async () => {
            if (!token) return
            try {
                const data = await animalsApi.getMyAnimals(token)
                setAnimals(data)
            } catch (error) {
    ...
    if (isLoading) {
        return (
            <div className="min-h-[60vh] flex items-center justify-center">
                <LoadingSpinner size="lg" text="Carregando seus animais..." />
            </div>
        )
    }
        ...
        <div className="min-h-full">
            ...
            >
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-bold text-foreground">Meus Animais</h1>
